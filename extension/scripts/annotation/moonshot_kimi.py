...
def _iter_sse_data(response):
    ...
    data_lines: List[str] = []
    # Moonshot's SSE response may omit an explicit charset. Letting requests
    # infer the encoding can turn UTF-8 punctuation into Latin-1 mojibake,
    # which then breaks the verbatim quote checks. Read complete lines as
    # bytes and decode them explicitly as UTF-8.
...
def generate_annotation(prompt_name: str, dialogue: dict,
    ...
    try:
        ...
        try:
            ...
            errors = validate(
                obj, dialogue["units"], require_scan=(prompt_name in SCAN_PROMPTS),
