...
Steps 1 to 8 decide whether an error exists and how many; steps 9 onward decide
what each one is and what the unit's cells say. Run them in order. Most
mislabelling comes from entering the family questions with a token that should
never have been founded, or with two faults collapsed into one, so the founding
steps are not preliminaries to be skimmed on the way to the interesting part.
...
3. **Fix the prevailing baseline, then mark every departure from it.** The
   baseline is the stated problem's mathematics by default, and the
   belief-conditional chain wherever a live thread's belief governs the work,
   as the token-localisation rule provides. Walk the unit line by line, and
   within each line component by component, and mark
   every value, equality, selection, role assignment, or omission that is false
   against that baseline. Do this exhaustively before considering what any of
   them mean: a departure noticed only after the story has been settled is
   usually the departure that gets absorbed into the story wrongly.

   A correct intermediate does not clear the line that produced it. Two
   departures inside one expression can cancel, so a value matching the
   intended solution is not evidence that its components are sound. Check every
   component of a line against the anchored meanings independently, and only
   then ask whether the line's output is right. Where the output is correct but
   a component is false under the anchored meanings, the component founds its
   token and the correct output is coincidence. This is the layered case, and
   it is where enumeration most often stops early, since an annotator who has
   found a reading on which the line comes out right will not look for a second
   departure inside it.

   Nothing
   false against the baseline means no token in this unit, and the unit's cells
   are decided by the credit procedure alone.
4. **Test each departure against the available-reading defence.** Where the
   problem is silent on a parameter, or its text supports more than one reading,
   a student-supplied fill or reading that accounts for the departure founds
   nothing: record it as a granted reading and drop the departure from the
   inventory. Availability is a property of the problem text, not of the
   student's fluency, and it is settled by the ambiguity defence's three
   conditions. A departure that survives this step is chargeable; one that does
   not is never charged, however clearly the annotator can reconstruct the
   belief behind it, since reconstructability establishes what the student
   thinks, not that the problem forbade it.
5. **Test each surviving departure against the live threads.** A departure whose
   value the belief of a live thread generates is that thread's downstream
   shadow or a fresh exhibition of it, not a new token. A departure that lands
   on a live thread's protected output while its own stated derivation mutates
   around it is that thread's defence material, folded into it as an exhibition
   and founding nothing. Only a departure no live thread accounts for proceeds.
6. **Test the surviving departures against each other for independence.**
   Correct one counterfactually while holding the student's other commitments
   fixed:
   ...
   Independence is a question about mechanisms, not about values. Where
   correcting one fault would make a second fault's value coincide with the
   corrected chain, the second act has been rendered invisible rather than
   removed, and it survives as an independent token; the tell is that the first
   fault's belief cannot itself generate the second act's value. In particular,
   a conceptual error does not absorb a separate equality that is false under
   the student's own stated quantities, and a value that is true under the
   intended reading is still a token where it is false under the student's own
   established chain. Conversely, arithmetic that is correct under a
   demonstrated wrong belief is only that belief's downstream shadow.
7. **State each surviving token's belief in checkable form.** Write what the
   student is committed to, as a claim she could be asked to affirm, at the
   grain at which it can recur and in the form its family's construct takes.
   - *Comprehension:* a reading, what the student takes the problem to say,
     mean, or ask.
   - *Relevance:* a selection rule, which of the problem's stated information
     she takes to fill a named role and on what basis she assigns it. The role
     and the basis both appear, since a belief naming only the misplaced
     quantity cannot match the complement once she corrects one side.
   - *Principles:* the deviant relation she applies in place of the correct
     one, with the governing structure named and the component she assigns
     wrongly identified within it. A belief naming only the wrong answer, or
     describing the arithmetic without identifying the relation, has not met
     the structural-fidelity test.
   - *Wrong operation:* the invalid construction, which construction she
     builds and how she selects or arranges its operands.
   - *Steps:* the shape she takes the procedure to require, naming the step
     omitted, the step added, or the ordering she treats as valid.

   Across all five the belief is stated generally enough to be re-applicable
   to a fresh instance and specifically enough that an instance handled
   correctly would count against it. A capability or disposition, that the
   student cannot simplify equations, is careless with signs, or is unreliable
   with fractions, is not a belief and may not occupy the field, being
   unfalsifiable within the dialogue, absorbing unrelated faults, and offering
   the re-emergence matcher nothing to run on. Where successive manifestations
   share no reconstructable procedure but do share a committed output, the
   thread is output-anchored under the anchored-composition clause and the
   signature names the output; where they share neither procedure nor output,
   they are distinct threads. For a candidate comprehension token the fuller
   treatment is at "A comprehension belief is a reading"; if the sentence
   contains an equality or a computed total it is a description of her
   arithmetic, not a belief, and the token has not yet been understood well
   enough to route. The arithmetic a belief produces belongs in the thread's
   signature as protected output, never in the belief itself.
8. **Scope each token's signature to what its thread will re-exhibit.** The
   signature names the content whose re-commitment constitutes re-exhibition,
   and nothing wider. Where several threads share a chain, an output produced
   jointly by them is claimed by the thread that defends it downstream, and a
   token whose fault is confined to one act takes a signature naming that act
   rather than the outputs it feeds. A signature claiming outputs its own thread
   does not defend will fire the re-emergence matcher on turns that exhibit a
   different thread. Where the belief is an assignment, a partition, or a
   selection with two or more sides, the signature names the assignment itself
   rather than one side of it, since a signature naming one side stops matching
   as soon as the student corrects that side alone, and the uncorrected
   remainder then presents as unattributed residue.

   Write the signature from the founding unit's prefix and do not revise it
   once later units are read. Where the founding unit is being revisited after
   the dialogue has been worked through, draft the signature from that unit
   alone before consulting what followed, then check the wording against a
   simple test: could this have been written knowing only the founding unit? A
   signature listing alternative forms, or hedging between them, usually fails
   that test, because the alternatives are the later turns showing through. The
   founding unit may itself contain two forms of one thing, so treat the
   disjunction as a flag rather than a bar, and satisfy yourself that both
   forms are on the founding unit's own page. A later turn the signature as
   founded does not cover founds its own thread; it is never a reason to
   broaden the wording.

   When the founding-time test fails, the remedy is never to reword. Restore
   the wording the test says you should have written, then re-run the later
   turn against it. If it no longer matches, that turn is not an exhibition,
   and it takes whatever labels its own content earns; where its content opens
   a family's gate it founds a new thread there, and where it does not it is
   judged by the ordinary floors. Reaching for a broader identity criterion at
   this point reverses the order of the tests, since identity is judged against
   the signature and cannot be used to rescue a signature the test has just
   rejected. A non-matching turn is the expected outcome of a correctly written
   signature, not a defect in it: a student whose belief changes mid-dialogue
   should produce threads that stop matching, and an instrument in which every
   later turn matches something already open has lost the ability to detect
   that change.
9. **Route each independent token in this fixed order.** Stop for that token as
   ...
   1. **Own-chain demonstration.** If the prefix correctly labels, defines, or
      ...
      The bar lifts on an articulated contrary reading. It bars an
      annotator-constructed story, not a reading the student herself states,
      so where she articulates the contrary reading in her own words,
      unprompted by the tutor, the bar lifts and the act is judged on that
      articulation. Distinguish this from defence material by the arithmetic
      test of the migrating-component rule: the parts the student states must
      themselves produce the protected value for the account to be a defence.
      Asking only what is held fixed does not separate the cases, since a
      constant output with mutating parts describes both.
   2. **Information ownership.** Choosing the wrong member of competing
      ...
      redescription is not evidence, and the bar runs in both directions: a
      redescription is no more evidence against a token than for a belief, so a
      committed value false under the student's own stated quantities founds
      its token even where an ingenious rereading, unmarked in her text, would
      render it true. This test is unreachable while an
...
10. **Fix family at origin, then track prospectively.** Assign a thread's family
...
11. **Run authorship and engagement separately for every family.** First ask
...
12. **Decide credit for each family in this fixed order.** Credit is decided
   act by act, not unit by unit. Before setting any family to not_evidenced on
   a unit that contains an error, list the unit's acts separately, the
   modelling decisions, the selections, the constructions, the sequencing, and
   ask of each which family's construct it engages. Only the token's own
   derivation is removed from the credit ledger; every other act is judged on
   its own merits, and an act correctly performed earns absent even where the
   unit's answer is wrong and even where a later act contradicts it. The
   presence of an error is not evidence about the other families, and a unit
   may carry a present cell and several absent cells at once. Reaching
   not_evidenced for the remaining families because the unit is wrong is the
   same failure as reaching absent for them because the unit is right. This
   procedure
   mirrors the token-routing order of step 9: run the tests in sequence, stop
   ...
   2. **Supply.** The authorship matrix of step 11. A supplied construct is
...
13. **Audit before output.** Confirm that:
   ...
   - every comprehension belief reads as a claim about the problem situation,
     and no belief field states an equality or a computed total;
   - every charged departure was tested against the available-reading defence
     before it was founded, and every granted reading is recorded as granted;
   - no token was founded on a value that a live thread's belief generates, or
     on a component migrating around a live thread's protected output;
   - every thread's signature names only content that thread will re-exhibit;
   - the thread set is jointly holdable, no two live threads assigning
     different values or incompatible roles to the same quantity at the same
     point in the chain;
  ...
  - **Floors evaluated before the gate (order of operations):** the engagement
    ...
    **not_evidenced** regardless of how much reasoning it displays.
    ...
    - *The bare-execution floor (principles):* bare execution of an arithmetic
      ...
      factor or rate merely appears inside arithmetic. A base named by the
      problem or by the tutor is supplied, not chosen: applying a stated rate
      to a base the text identifies is execution and engages the operation
      construct at most, however much conceptual vocabulary the student
      wraps around it. Deciding what an ambiguous phrase denotes is
      comprehension's construct, so a turn that settles which quantity a
      phrase refers to is judged there and never credited as a principles
      choice.
...
- **Reconstruct the student's mistake before labelling ambiguous turns
  ...
  **Two guards on this practice.**
  ...
  - **Reconstruction is anchored to the student's expressed semantics.**
    Candidate belief models are built from the semantics the student's own
    words and written expressions encode. Begin at her most explicit
    expression of the contested quantity, a written formula or a labelled
    derivation, fix the model there, and test the rest of the chain against
    it; a mismatch downstream founds a token and is never a licence to
    reinterpret the anchor. Where her wording gives a quantity one consistent
    meaning across the chain, a duration, a count, a position, a rate, a role,
    every candidate model carries that meaning, and a model that reinterprets
    the quantity against consistent wording is unavailable unless her text
    positively marks the alternative. Internal consistency of the resulting
    chain never establishes availability, for readings of the student's chain
    exactly as for readings of the problem: a chain can be run flawlessly on a
    model she never held. Parsimony accordingly compares available models
    only, and unification bought by an unmarked reinterpretation is annotator
    construction, not the single belief the parsimony rule prefers. This
    constrains reconstruction across a chain and leaves the
    stated-versus-enacted clause below untouched, which continues to govern
    conflicts inside a single claim between narrated operation and enacted
    result.
  - **Which expression anchors.** The anchor is the earliest unit in which the
    student uses the contested term or role inside a fully worked expression,
    one whose components are all written out. A term so used carries that
    meaning for the rest of the chain, and a later occurrence requiring a
    different meaning is a departure, not a redefinition. Where two
    expressions compete, prefer the one whose components are written over the
    one whose value merely fits, since a value can fit for reasons that have
    nothing to do with the meaning in question. This applies with particular
    force to role words the student supplies herself, such as gained, left,
    spent, or remaining: once she has used such a word in a worked expression,
    a later use of the same word denotes the same role, and reading it as a
    different role to make a later line come out coherent is annotator
    construction.
  - **Incoherent turns (the three-limb boundary).** Where a turn's content is
    ...
    relabelled, per causal scope. The upgrade fires on a repeated form, not
    on a repeated value. What repetition establishes is a route, since the
    same manipulation applied twice is a procedure however garbled, and the
    thread founded this way takes that form as its signature. A value
    re-asserted while the derivations around it mutate establishes no route:
    it is an anchor rather than a form, and where that anchor is itself
    unattributed the orphaned-anchor rule governs and nothing founds.
  ...
  - **A derivation you can point to is not a reading you can state.** These
    are two different findings, and a turn can have the first without the
    second. A derivation is recoverable when the student's committed claim can
    be traced to quantities already on the page, including as a manipulation
    of her own earlier work; that is what the route test asks, and it is what
    a procedural or output-anchored thread needs. A reading is statable when
    the student has said, or her work has shown, what a quantity or expression
    *is* in the problem, what role it fills, what it denotes, what it is meant
    to measure; that is what a comprehension thread needs. Where a turn is
    recoverable but states no reading, treat the two findings separately.
    Route the recoverable content through the fixed order on its own merits.
    Leave comprehension not_evidenced, and do not supply a reading to fill the
    gap, however natural the reconstruction seems: an annotator's account of
    what the student must have meant is not evidence, and a turn that shows
    only symbols has not told you what she takes them to mean. Where a later
    turn states the meaning, that turn founds its own comprehension thread
    from its own prefix, and the earlier turn is not relabelled.
    Recoverability is necessary for every family and sufficient for two. For
    wrong operation and steps the derivation is the construct, so a
    recoverable claim is judged there directly. The three remaining families
    each require something the derivation does not supply, and each has its
    own gate: comprehension a statable reading, per this rule; relevance an
    articulated selection, per the silence floor; principles a structurally
    faithful relation, per the structural-fidelity admission test. Where a
    turn is recoverable and the relevant gate does not open, the finding is
    not_evidenced for that family, never a reconstruction supplied to open it.
  - **An articulation that defends prior work is still an articulation.** A
    student asked to justify an act will state the belief behind it, and that
    statement is the positive evidence the admission gate requires. That the
    articulation serves to defend what she has already written does not
    discount it, since a belief is no less held for being invoked under
    challenge, and the *rationale* event tag exists for precisely this event.
    Discount an articulation only where the student's own work contradicts it
    in the same turn, where its content was supplied by the tutor, where it
    names no role or meaning, or where the parts it states themselves produce
    a protected output, which makes it defence material under the
    migrating-component rule. That list is closed. Do not discount it on the
    ground that the act it
    explains was garbled: an explanation of a garble is how a garbled act
    becomes attributable, and it is often the only place a belief is ever
    stated.
  - **Founding follows the gate, not the earliest suspicion.** A thread founds
    at the first unit where its family's gate opens on that unit's own
    content, not at the earliest unit where the annotator can imagine the
    belief already operating. Where an earlier unit enacts something the gate
    did not open on, that unit keeps the label its own content earns and is
    never relabelled once the later unit founds. An annotator who cannot found
    at the earlier unit and therefore declines to found at the later one has
    lost the token to a precedence rule that does not exist: the priority
    given to a structurally diagnostic act settles which evidence suffices,
    never that founding must be pushed back to a unit whose content fails the
    gate.
...
**A thread's belief and signature are fixed at founding.** Both are written
from the founding unit's prefix and are not edited afterwards, as the family
field is not edited. A later turn either matches the thread as founded or it
does not. Widening a belief or loosening a signature so that a non-matching
turn matches is later evidence entering through the thread definition, which
causal scope forbids as surely as it forbids relabelling the earlier unit, and
it is the same failure whether the widening is done deliberately or discovered
by rereading the founding unit in the light of what followed. Where a later
turn exhibits a belief the founded one does not cover, it founds its own
thread.

**Uniform reading of this instrument.** Apply the codebook's current text
uniformly to every unit annotated. Where a clarification of how a rule reads
alters how earlier units should have been decided, revisit those units and
revise them, re-deciding each from its own prefix alone, so the finished
annotation reflects one consistent reading of the instrument rather than a
drift across the corpus. Evidence from later units never enters a revision of
an earlier unit's labels: a revision may correct how the rules were applied to
a prefix, never what the prefix is taken to contain.
...
thread's falsity swallowing the later token's location. This passage concerns
where a token sits, not whether its content is a misconception at all: the
arithmetic-slip exclusion and the correctness-failure clause run first, and a
citation here is not authority for charging a bare miscalculation. What makes the second
act a token is that it departs from the prevailing baseline, not that its value
differs from the intended solution's: a value that happens to be true under the
intended reading is still a token where it is false under the student's own
established chain, since she has committed content her own work contradicts.
The independence test at protocol step 6 accordingly asks whether the first
fault's belief can generate the second act's value, not whether correcting the
first would make the second invisible.
...
- **Sentence replay is recitation; genuine reformulation is a new trial.**
  ...
  in place of them.
  **Naming an operation the prior chain already displayed is narration, not
  reconstruction.** Where the earlier work shows an equation and the current
  turn states in action form what that equation already displays, that she
  added these operands or divided by that figure, the naming reports copied
  content rather than independently assembling operands. Connectives marking
  sequence, first-person framing, and recasting a labelled result as the act
  that produced it are among the changes that leave a turn recitation, and an
  account that supplies all three at once is still reciting. Freshness
  requires a relation, premise, or organisation the prior chain does not
  contain. Where an account's content sentences correspond one-to-one with
  the prior work's in the same derivational order, run the comparison at that
  level before crediting any column, since fluent narration of a copied chain
  reads as explanation while retrying nothing.
  **Reassembly after a correction is fresh.** Where a component of an
  aggregate is corrected and the student rebuilds the aggregate around the
  new value, the reassembly is a fresh act for the family whose construction
  it exercises, even though the aggregate's structure appeared earlier in the
  chain. That family is wrong operation. Assembling operands into a total, or
  putting a corrected quantity through a stated rate or conversion, is a
  construction built within a step and it earns there. Steps is not engaged by
  a reassembly that reuses the prior chain's stages in the prior chain's order;
  it earns only where the student contributes a stage or an ordering the prior
  chain does not contain, such as a rounding or a unit conversion the earlier
  work never needed. The act carries its own opportunity to fail, most
  obviously by dropping a component, double-counting one, or retaining the
  superseded value, and an opportunity to fail is what the credit tests exist
  to identify. The carried default reaches the components themselves, whose
  values the prior chain does contain, and not the assembling act. Credit
  follows the construction actually performed, so the concept-level relation
  behind the corrected component, supplied or already settled upstream, earns
  nothing of its own.
...
- **When the tutor is wrong (wrong premises, endorsements, and models).** The
  ...
  - **An imprecise tutor turn is judged by what it licenses, not only by what
    it states.** A tutor's guidance may be true yet incomplete, and the student
    may draw from it an inference the phrasing invites but the tutor never
    asserted. Where the student's move is a defensible reading of the tutor's
    turn, the resulting content is the tutor's contribution and is not present
    for any family, even though the framing floor does not fire, since the
    floor asks what was stated while this asks what was licensed. Nothing earns
    on that content either, the interpretive work having been the tutor's.
    Where the move is not a defensible reading of the tutor's words, it is the
    student's own and is judged normally. This mirrors the ambiguity defence,
    applied to the tutor's text rather than the problem's, and it expires the
    same way, so once the tutor makes the intended point explicit, persistence
    in the alternative reading is the student's own.
...
- **A wrong value stated beside its own correct result is a correctness
  failure, not a misconception.** Where the student selects the right
  construction, arranges the right operands, **states its correct result**, and
  holds no statable misreading, but then commits a different value as the
  answer, all five families are not_evidenced and no thread founds. The
  wrongness is carried by the correctness record, which is what the channel's
  independence from correctness exists for. The clause requires the correct
  result to be visible on the page beside the committed value; that visible
  result is what establishes the execution as sound. Where the committed value
  is itself what the student's execution produced, the execution was not sound
  and the act routes through the fixed order as usual: a sign dropped while
  collecting terms, a term mis-collected, an operand misplaced or omitted is an
  arrangement fault and belongs to wrong operation. The clause also reaches a
  committed value that is simply a miscomputation of the construction the
  student has written, whether or not the correct result also appears on the
  page. What separates that from an arrangement fault is the expression as
  written: where the expression is the right one and only its evaluation is
  wrong, no family's construct has failed and no thread founds; where the
  expression itself is misbuilt, the fault is in the construction and routes to
  wrong operation. This is not a licence to
  withhold labels, and "sound operation" does not mean that the right kind of
  operation was chosen; it means the operation was carried out and its correct
  result written down.
- **Motivated false arithmetic is perseveration, not a slip (answer-anchoring).**
  ...
  **present** for the family whose belief the falsehood protects, reconstructed
  ...
  misconception defending itself, not noise. **An orphaned anchor founds
  nothing.** This carve-out makes the assertion the act of the thread whose
  output it preserves. Where the preserved value is itself unattributed, no
  thread protects it and the carve-out has nothing to attach to: record the
  assertion in the note, and it neither founds a thread nor exhibits one. Five companion judgments keep
  ...
  - **A migrating component is defence material, not a belief exhibition.**
    Where a committed total persists across turns while the stated parts
    around it mutate, each mutation keeping the sum at the protected value,
    the parts are manufactured components of that output's defence. They fold
    into the defended thread's exhibitions and found nothing of their own,
    because a belief exhibition requires its value to occupy a stable role,
    and the admission tests fail for a value that migrates between roles from
    turn to turn or contradicts the student's own articulation in the same
    sentence. A genuinely held reading also predicts its own consistent total,
    so the sustained absence of that total across the defence weighs against
    founding, and the invariant output identifies which thread is live. The
    test is arithmetical, not impressionistic: a defence's mutated parts must
    themselves produce the protected value. Where the parts the student states
    do not produce it, and a further false equality is needed to bridge the
    gap, the parts are not manufactured for the defence. They are judged on
    their own merits and may found, while the bridging equality is the
    protecting thread's act. A constant output with mutating parts describes
    both the defended and the believed case, so it does not by itself
    discriminate them; only the arithmetic does.
...
- **Ambiguity defence (a defensible reading of an ambiguous problem is not the
  ...
  - **Genuine ambiguity:** the problem text itself supports more than one
    ...
    natural. Where the problem is *silent* on a parameter rather than
    ambiguous about a stated one, a starting value or an initial state it
    never fixes, the student's fill is available only where the scenario's own
    characterisation leaves that value open. The granted silent-parameter
    defences are of this kind, the problem characterising neither value. Where
    the narrative characterises the situation in a way that weighs
    determinately against the fill, a capability read into a subject the
    problem describes as lacking it, a resource read into a situation
    described as without it, the fill is a fabricated scenario assumption and
    charges comprehension when enacted. Physical possibility and internal
    consistency do not establish availability against the scenario's own
    characterisation, and the annotator's ability to reconstruct the fill
    clearly is not evidence of its availability.
...
3. **Thread-linked resolution.** A resolution is the student's own act
   ...
   chain, is noted, not present). Isomorph credit is subject to the analogy
   gate stated in the labelling section: a success earns credit floors only
   where the analogy is capable of eliciting the student's misconception,
   which is what makes it an isomorph rather than an illustration. A worked
   example posed to prompt insight, testing only whether the student can
   perform a basic operation, engages no construct and is not_evidenced
   however correct the answer, and that holds for credit exactly as it holds
   for presence. Isomorph credit also
...
4b. **Individuation and the residual scan.** Threads are individuated by
   ...
   Before founding on residue, test it against the mirror of every live belief:
   the other side of an assignment, the remainder of a partition, the balance
   of a total, the complement of a selection. Correcting a belief corrects its
   mirror, so mirror content is that thread's exhibition and founds nothing,
   and residue that survives only because the counterfactual correction was
   applied to one side of a two-sided belief is not residue at all.
...
**The burden default decides ties, not absences.** It applies where two
competing reconstructions each fit the evidence and the choice between them
cannot be made, which is the case its worked example describes. Where no
reconstruction fits, no reading is statable under the comprehension belief
rule and no procedural fault is identifiable, the default does not apply. An
unstatable belief is not a conceptual one by default, and the absence of a
reading is not evidence of a misreading.
...
A substitute for this purpose is content the student supplies that the problem
does not state, a rate, relation or rule. Naming the quantity actually used is
not enough, since every narrated subtraction names its own minuend. A label
attached to the quantity used, per weekend, in total, for the year, is part of
naming that quantity and is not a stated substitute belief. The tie-breaker's
comprehension arm requires supplied content of the rate-relation-rule kind;
where the only candidate substitute is the wrong label itself, the tie-breaker
does not fire and the misassignment routes by the dimensional-slot clause. Where the
vacated slot is filled by another of the problem's own stated givens of the
same kind, the student has misassigned which given attaches to the condition
rather than stated a wrong model, the relevance arm governs, and this
tie-breaker does not fire. Where it is filled by a mislabelled derived
quantity, the registration note's derived-mislabel account governs and files
comprehension.
...
structure misread. The discriminator is whether incorporating the condition
would change the shape of the computation or only supply a factor or term
within it. A count of the occasions on which a described event happens is a
value: it enters as a multiplier and leaves the chain's shape untouched. A
tier, a conditional deduction, or a qualifier that makes a different
computation apply is a rule (see "Misreading a described structure is comprehension" in
...
- **Same-kind confusion is relevance.** A confusion between two quantities of
  ...
  thing the slot needs and picks the wrong instance of that kind. This clause
  does not require two stated candidates; that requirement belongs to the
  stated-conditions tie-breaker, which this clause exists to supplement. It
  applies wherever a single quantity, stated or derived, is attached to the
  wrong same-kind slot, including the wrong timeframe: one set of stated
  amounts attached to the weekend where the text attaches them to the day is a
  same-kind misassignment and files relevance, however the student phrases the
  attachment.
...
**What the student believes includes what she takes a stated condition to do.**
The beliefs this question ranges over are not only beliefs about what the
quantities are, but beliefs about the direction and role of the problem's
stated conditions: whether a condition adds or removes, repeats or restricts,
scales up or down. An act that is incoherent on every belief about the
quantities may still be the correct enactment of a belief about a condition's
direction, and where the student states that belief the act is right on her own
logic in the sense this question intends. Searching only over readings of the
quantities and concluding "wrong on any belief" is therefore an incomplete
application of this test. Ask what her rationale says the condition does, not
only what her chain says the numbers are. Where she states that a condition
reduces and she divides, the belief is the construal and the family is
comprehension; where she states that it increases and divides anyway, the
belief is intact, only the move is inverted, and the family is wrong
operation. This unifies the two family-level rules that already govern this
...
**A wrong-direction operation with a stated rationale routes by what the
rationale construes.** Where the rationale construes a stated condition as a
different kind of thing than the text makes it, a repetition clause taken as a
restriction, a growth condition taken as a cap, the construal is the belief,
the thread files comprehension, and the inverted operation is its enactment.
The construal need not make the operation arithmetically coherent, since the
founding standard is a demonstrable reading, not a rationalising one, and the
belief field states the construal, never the operation. A construal is demonstrated by what the rationale says the
condition does, not by whether her restatement of the condition is factually
true: "only two days each weekend" restates a true fact and construes it as a
restriction, and the truth of the restatement is not the test. Where the
rationale shows the condition construed correctly in its direction and role and
only the move inverted, or where the rationale says nothing about what the
condition does, the inversion is wrong operation, which is where the 1538 back-projection sits, its timeframe
construal already held by its own thread. Persistence past the student's own
correct performance of the counterpart operation, and reassertion of the
inverted move in the same breath as naming its corrected counterpart, are
evidence the driver is a construal rather than a move choice, and must be
consulted before the procedural filing is made.
...
still-needed aggregation, it is wrong operation. That precedence is conditional
on its own terms: where the aggregation is not still needed, the row does not
apply at all and its precedence does not arise. Precedence orders rows that both
apply; it does not revive a row whose condition has failed. Steps is reserved
for adding a redundant derivation or stage at the level of the whole. This distinction is
...
an excess operand, wrong operation. The form of the later act is not the
discriminator: an act that fills a role already filled is unlicensed whether it
is a repeat of the same computation, a fresh aggregation, a scaling, or any
other construction. "A closing re-derivation" above names the role status, not
the arithmetic shape, so do not conclude that the steps arm is inapplicable
because the act is a sum rather than a repeat. The same test separates a label naming
...
cluster test, wrong on her own logic is procedural). The cluster test separates
the conceptual families from the procedural ones and presupposes that some
construct has failed. It does not convert a bare miscalculation into a
procedural token: where the construction is correct as written and only its
evaluation is wrong, the correctness-failure clause governs and no family is
present.
...
**A comprehension belief is a reading.** Before founding a comprehension
thread, restate its belief as what the student takes the problem to say, mean,
or ask: a stated quantity taken as a different kind, structural level,
referent, or requested role; a described structure misread; a goal misread; or
a scenario assumption fabricated without textual basis. Where the restatement
cannot be made, no reading has been identified, the content is not
comprehension whatever its surface resembles, and the act returns to the fixed
routing order. Where it can, check the reading against the ambiguity defence
before founding, since an available reading charged as an error is a founding
failure rather than a misconception. The totals and boundaries a reading
produces belong in the thread's signature as protected outputs, never in the
belief, and a belief field containing an equality or a computed total has
described the student's arithmetic instead of her reading.
...
**This rule presupposes an established level.** It applies where the problem
states the quantity's level, as the rotten kilograms are stated once for the
whole batch, or where the student's own chain establishes it before the act
that misapplies it. Where the only thing in the problem establishing the level
is a condition the student never registered, there is no established level for
her to misread: strip that condition and her chain is the one she would have
built with the condition absent, which is the starved-structure case, and the
ignored-condition test governs instead. The discriminator is registration. A
student who uses the condition and then mislabels what her own use produced has
established the level in her chain and misread it, which is comprehension; a
student in whose chain the condition never appears has been starved of the
datum that would have fixed the level, and the default label she reaches for is
the omission's consequence, not a construal of a level. Check before applying
this rule whether the level would survive deleting the unregistered condition;
if it would not, this rule is not the one.
...
**A prompt that performs the decomposition supplies the concept.** Where the
tutor's turn states the part-whole reading itself, naming what a quantity
amounts to in parts of a whole, or which fraction of what it represents, the
concept-level work is his and the student's arithmetic on it is execution.
Principles is not_evidenced there however self-assembled the calculation looks,
and the same holds on the target and on a tutor-posed isomorph alike. This is
distinct from a prompt that names a target, or asks what comes next, while
leaving the student to decide what the quantity represents, where the
decomposition is hers and earns. The test is whether, after the tutor's words,
the student still had to determine what the quantity is a part of.
...
1. **Evidence:** for every comprehension thread, what exact student statement,
   ...
   belief and reroute the observable act. Confirm also that each comprehension
   belief is stated as a reading, a claim about what the problem says, means,
   or asks, and that no belief field states an equality or a computed total in
   place of one.
2. **Founding:** for every charged departure, was the available-reading defence
   tested before the token was founded, and is every granted reading recorded
   as granted? Confirm that no token was founded on a value a live thread's
   belief generates, or on a component migrating around a live thread's
   protected output, and that each signature names only content its own thread
   will re-exhibit.
3. **Provenance:** what were the competing inputs at the instant of each
   disputed selection? Confirm that no later transformation changed their
   historical source class.
4. **Roles:** which predicates has the student assigned to every produced
...
5. **Residual errors:** after correcting each attributed token alone, does any
   false equality, assignment, selection, or unnecessary act remain? Route each
   survivor as its own thread.
6. **Procedure grain:** for every operation/steps boundary, was the exact target
...
7. **Credit:** for every absent cell, what construct-level decision did the
...
8. **Temporal integrity:** did later evidence alter an earlier family or
...
9. **Coherence:** read the thread set as a whole. Could one student hold all
   of these beliefs at once, at the units where they overlap? A pair that
   assigns different values, or incompatible roles, to the same quantity at the
   same point in the chain is not a pair of beliefs; at least one is an
   annotator's misreading. Where the set fails this, return to reconstruction
   and prefer the account on which the student's own work is internally
   consistent, since a reconstruction that has her contradicting herself loses
   to an available one that does not. Threads that succeed one another because
   the belief changed are not in conflict; the test applies only where two
   threads are live over the same unit.
10. **Commitment:** where two readings survive every test above, apply the
