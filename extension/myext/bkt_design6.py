"""
myext/bkt_design6.py

Design 6: TEMPERED FAMILY-CONDITIONED emission. The Design 5 tempering
(correctness vs misconception weight) applied to Design 2a's family-specific
misconception emission.

For a split (a, b) with a + b = 100 and family F, the emission for mastery state
k is

    P(obs | k, F) = [P(C_t | k)]^{w_c} * [P(M_t | k, F)]^{w_m}

with w_c = 2a/100, w_m = 2b/100 (sum-to-2 convention, as in Design 5).

Anchoring:
    (100, 0)  -> correctness only (misc factor = 1; family becomes irrelevant,
                 so this collapses to the plain correctness baseline).
    ( 50,50)  -> exactly Design 2a (untempered family-conditioned emission).
    (  0,100) -> family-specific misconception only.

This is "Design 5 for 2a". The weight tunes the correctness-vs-misconception
balance; the misconception channel is family-specific throughout. It does NOT
sweep the pooled-vs-family-specific axis (that is Design 7).

The core BKT parameters (prior, learn, guess, slip) are pooled across families,
exactly as in Design 2a; only the misconception emission is family-specific.
The M-step uses Design 2a's closed-form updates with tempering acting through
the reweighted responsibilities (same modelling decision documented for
Design 5).

Fit:   fit_design6(train_long, granularity, split=(a, b))
...
from . import bkt_design5 as d5   # reuse split_to_weights, tempered emission/FB
...
def _em_single_kc(sequences: List[dict], w_c: float, w_m: float,
    ...
    """EM for one KC: pooled core params, per-family mu0/mu1, tempered emission."""
    ...
    for _ in range(n_restarts):
        ...
        for _it in range(max_iter):
            ...
            for seq in sequences:
                fam = seq["family"]
                gamma, xi, ll = d5.forward_backward_tempered(
                    seq, prior, learn, guess, slip, mu0[fam], mu1[fam], w_c, w_m)
                total_ll += ll
...
class FittedDesign6:
    ...
    def predict_long(self, long_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for (_did, skill), grp in df.groupby(["dialogue_idx", "kc"], sort=False):
            ...
            preds[pos] = d5.predict_seq_tempered(
                grp["correct"].to_numpy(dtype=int), grp["misc"].astype(str).to_numpy(),
                self.granularity, core["prior"], core["learns"], core["guesses"],
                core["slips"], mu0, mu1, self.w_c, self.w_m)
        out = df.copy(); out["pred"] = preds
        return out.drop(columns="_turn_num").sort_index()


def fit_design6(train_long: pd.DataFrame, granularity: str,
                split: Tuple[float, float], n_restarts: int = 4,
                seed: int = 221, verbose: bool = True) -> FittedDesign6:
    """Fit Design 6 (tempered family-conditioned emission) at a given split."""
    w_c, w_m = d5.split_to_weights(split)
    ...
    if verbose:
        print(f"[design6] tempered family-conditioned, split={split} -> "
              f"(w_c={w_c:.2f}, w_m={w_m:.2f}), granularity={granularity}: "
              f"{len(rows)} KCs fitted")
    return FittedDesign6(per_skill=rows, fallback=fallback, granularity=granularity,
                         w_c=w_c, w_m=w_m, split=split)


# same split grid as Design 5
DEFAULT_SPLITS: List[Tuple[int, int]] = list(d5.DEFAULT_SPLITS)
